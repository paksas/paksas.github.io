...
He will then toss it one more time, and you need to guess the result. If you win, you get 10$. If you loose - you loose 5$.
...
* 5 Heads and 5 Tails - $$E[X] = 0.5$$, so... make a guess \_O_/
...
Our coin tossing is best represented by a Random Variable with a Binomial Distribution.

We remember that a discrete Random Variable is a sampler that pulls items out of a set. How frequently will certain values show up is dictated by its probability (mass) function. And it's expected value (or mean) indicates which particular item we can expect to see pulled out the most.
